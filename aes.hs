import qualified Data.ByteString as B
import Data.Word
import Data.Bits
import Data.List.Split

type Block = [Word8]
type Key = [Word8]

blockSize :: Num a => a
blockSize = 16

numRounds :: Num a => a
numRounds = 10

sBox :: [Word8]
sBox = []

-- For now we use this key as every round key.
-- The Round Key generation infrastructure should probably be a monad
rootKey :: Key
rootKey = [0x88, 0x9d, 0x8a, 0x79,
    0x99, 0xf4, 0xe8, 0x4b,
    0xb5, 0x4f, 0xd0, 0xda,
    0xaf, 0x5f, 0xf1, 0x43]

addPadding :: [Block] -> [Block]
addPadding bs = init bs ++ (padded . last $ bs)
    where
        repNum n = blockSize - 1 - n
        padded b
            | length b == blockSize = [b] ++ [0x80 : replicate (repNum 0) 0x00]
            | otherwise = [b ++ [0x80] ++ replicate (repNum $ length b) 0x00]

-- Electronic Codebook Mode - Does not chain output with XOR
encryptBlocksECB :: Key -> [Block] -> [Block]
encryptBlocksECB key = map $ encryptBlock key

encryptBlock :: Key -> Block -> Block
encryptBlock key block = (iterate (encryptRound key) initialState) !! (numRounds - 1)
    where
        initialState = keyAdd key block

encryptRound :: Key -> Block -> Block
encryptRound key = keyAdd key . mixColumns . shiftRows . byteSub

keyAdd :: Key -> Block -> Block
keyAdd = zipWith xor

mixColumns :: Block -> Block
mixColumns = id

shiftRows :: Block -> Block
shiftRows = id

byteSub :: Block -> Block
byteSub = map (\x -> sBox !! fromIntegral x)

main = do
    plaintextBlocks <- chunksOf blockSize . B.unpack <$> B.readFile "inputtext.txt"
    plaintextBlocks' <- return [last plaintextBlocks] -- Remove later
    encryptedBlocks <- return . encryptBlocksECB rootKey . addPadding $ plaintextBlocks'
    print encryptedBlocks
    B.writeFile "outputtext.txt" . B.pack . foldr (++) [] $ encryptedBlocks
